...
그래서 useEffect 를 통해 페이지 이동 마다 mount 되는 것을 확인했다. return 사용해 unmount 일어나는 것까지 확인하면 좋았을 것 같다.

그래서, 일단 이전에 구현되어 있던 부분을 layout 으로 통째로 옮김.

잘 적용되는 것 확인했고, 내가 담당한 Sidebar 영역도 하나의 컴포넌트로 만들어 코드를 더 세세히 나누고 싶었다.
...
상태를 정리하고 나니 레이아웃도 서버 컴포넌트로 만들 수 있게 되었다.
